import { checkIfSudokuIsSolved, selector } from './selector';
import { makeGrid } from '../gridCreator/gridCreator';
import { ENDGAME_TYPES } from '../store';

const grid = makeGrid();
const fullGrid = makeGrid('testFullPuzzle');

test('Селектор возвращает GAME_IS_NOT_OVER, если счетчик только 3 ошибки', () => {
  const result = selector(grid, 3);
  expect(result).toEqual(ENDGAME_TYPES.GAME_IS_NOT_OVER);});

test('Селектор возвращает FAIL, если счетчик достиг 5 ошибок', () => {
  const result = selector(grid, 5);
  expect(result).toEqual(ENDGAME_TYPES.FAIL);
});

test('Селектор возвращает WIN, если вся сетка заполнена и ошибок меньше 5', () => {
  const result = selector(fullGrid, 2);
  expect(result).toEqual(ENDGAME_TYPES.WIN);
});

test('Функция checkIfSudokuIsSolved возвращает false, если полученная на вход сетка имеет пустые клетки', () => {
  const result = checkIfSudokuIsSolved(grid);
  expect(result).toBe(false);
});
test('Функция checkIfSudokuIsSolved возвращает true, если полученная на вход сетка заполнена целиком', () => {
  const result = checkIfSudokuIsSolved(fullGrid);
  expect(result).toBe(true);
});
